...
export async function GET(req: NextRequest) {
  ...
  if (user.role !== 'PARENT' && user.role !== 'STUDENT') return NextResponse.json({ error: 'Forbidden' }, { status: 403 });

  try {
    let studentIds: string[] = [];
    if (user.role === 'PARENT') {
      const links = await prisma.parentStudent.findMany({ where: { parentId: user.id }, select: { studentId: true } });
      studentIds = links.map(l => l.studentId);
    } else {
      const student = await prisma.student.findFirst({ where: { userId: user.id } });
      if (student) studentIds = [student.id];
    }
    if (studentIds.length === 0) return NextResponse.json({ error: 'No students found' }, { status: 404 });
