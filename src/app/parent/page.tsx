    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50">
      ...
      <div className="max-w-lg mx-auto px-4 py-4 pb-8">
              ...
              <div key={g.id} className="bg-white rounded-2xl p-4 border border-slate-100 shadow-sm">
                ...
                {(g.avgRaw != null || g.highScore != null || g.lowScore != null) && (
                  ...
                  <div className="flex gap-3 mt-2">
                    ...
                    {g.avgRaw != null && <span className="text-xs text-orange-500 font-medium">평귰 {g.avgRaw}점</span>}
        ...
        {tab==='wrongAnswers'&&(<div className="space-y-3">
          ...
          {waStats && (
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <div className="bg-red-50 rounded-2xl p-3 text-center border border-red-100">
                ...
                <p className="text-xs text-red-400">�출결</p>
          ...
          {wrongAnswers.filter((wa: any) => wa.status === 'ACTIVE').length > 0 && (

            <button

              onClick={() => {

                const activeWAs = wrongAnswers.filter((wa: any) => wa.status === 'ACTIVE')
                ...
                activeWAs.forEach((wa: any) => {
                  ...
                  if (wa.testPaper?.answers) {
                    ...
                    try {
                      ...
                      if (!answersMap[wa.testName]) answersMap[wa.testName] = {};

                      Object.assign(answersMap[wa.testName], parsed);
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  body { font-family: 'Malgun Gothic', -apple-system, sans-serif; max-width: 800px; margin: 0 auto; padding: 16px; background: #f8fafc; }

  .header { text-align: center; background: linear-gradient(135deg, #3b82f6, #4f46e5); color: white; padding: 20px; border-radius: 16px; margin-bottom: 16px; }
  ...
  .btn-bar button { flex: 1; min-width: 100px; padding: 12px 8px; border: none; border-radius: 12px; font-size: 14px; font-weight: 600; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 6px; }

  .btn-back { background: #f1f5f9; color: #475569; }

  .btn-share { background: #fee500; color: #3c1e1e; }
  ...
  .problem-card { background: white; border-radius: 12px; overflow: hidden; border: 1px solid #e2e8f0; }

  .problem-num { display: flex; align-items: center; justify-content: space-between; padding: 8px 12px; background: #f8fafc; border-bottom: 1px solid #e2e8f0; }

  .problem-num .num { font-size: 14px; font-weight: 700; color: #2563eb; }

  .problem-num .source { font-size: 11px; color: #94a3b8; }

  .problem-img { padding: 8px; }

  .problem-img img { width: 100%; height: auto; border-radius: 8px; }

  .no-img { padding: 24px; text-align: center; color: #cbd5e1; font-size: 13px; }

  .answer-section { padding: 8px 12px; background: #eff6ff; border-top: 1px solid #bfdbfe; }

  .answer-section .label { font-size: 11px; color: #3b82f6; font-weight: 600; }

  .answer-section .value { font-size: 16px; font-weight: 700; color: #1e40af; margin-top: 2px; }

  .footer { text-align: center; margin-top: 20px; padding: 12px; color: #94a3b8; font-size: 12px; }
  ...
  <div class="header">
    ...
    <p>${studentName} | ${today} | 총 ${activeWAs.length}문항</p>
  ...
  <div class="btn-bar no-print">
    ...
    <button class="btn-share" onclick="shareTest()">공유</button>
  ...
  <div class="problems">
    ...
    ${activeWAs.map((wa: any, idx: number) => {

      const page = wa.testPaper?.pages?.find((p: any) => p.pageNumber === wa.problemNumber);

      const imgUrl = page?.imageUrl || wa.problemImage || '';

      const answer = answersMap[wa.testName]?.[wa.problemNumber] || answersMap[wa.testName]?.[String(wa.problemNumber)] || '';

      const circleMap: Record<string, string> = {'1':'①','2':'②','3':'③','4':'④','5':'⑤'};

      const displayAnswer = circleMap[answer] || answer;

      return `<div class="problem-card">

        <div class="problem-num">

          <span class="num">${idx + 1}번</span>

          <span class="source">${wa.testName} ${wa.problemNumber}번</span>

        </div>

        ${imgUrl ? `<div class="problem-img"><img src="${imgUrl}" alt="문제" /></div>` : '<div class="no-img">이미지 없음</div>'}

        <div class="answer-section">

          <div class="label">정답</div>

          <div class="value">${displayAnswer || '정답 미등록'}</div>
  ...
  <div class="footer">수학탐구 오답노트 | ${today}</div>
    ...
    function shareTest() {
      ...
      if (navigator.share) {
        ...
        navigator.share({
          ...
          title: '오답 연습 - ${studentName}',

          text: '${studentName} 오답 연습 테스트 (${activeWAs.length}문항)',
        ...
        alert('링크가 복사되었습니다. 카카오톡 등에 붙여넣기 하세요.');
              ...
              className="w-full py-3.5 rounded-2xl text-white font-semibold text-sm shadow-md transition-all active:scale-[0.98]"

              style={{background:'linear-gradient(135deg,#3b82f6,#4f46e5)',boxShadow:'0 4px 12px rgba(59,130,246,0.3)'}}

            >

              📝 오답 연습 테스트 보기 ({wrongAnswers.filter((wa: any) => wa.status === 'ACTIVE').length}문항)
                      ...
                      <div className="space-y-2 mt-2">
                        ...
                        {items.filter((wa: any) => wa.status === 'ACTIVE').sort((a: any, b: any) => a.problemNumber - b.problemNumber).map((wa: any) => {
                          ...
                          try {
                            ...
                            const parsed = wa.testPaper?.answers ? (typeof wa.testPaper.answers === 'string' ? JSON.parse(wa.testPaper.answers) : wa.testPaper.answers) : {};

                            answer = parsed[wa.problemNumber] || parsed[String(wa.problemNumber)] || '';
                          ...
                          return (
                            ...
                            <div key={wa.id} className="border border-slate-200 rounded-xl overflow-hidden">
                              ...
                              <div className="flex items-center justify-between px-3 py-1.5 bg-slate-50">
                                ...
                                {displayAnswer && <span className="text-xs font-bold text-emerald-600">정답: {displayAnswer}</span>}
